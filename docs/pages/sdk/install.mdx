...
Today, there are release candidates versions (labeled `-rc.XX`) and beta versions (labeled `-beta.XX`). 
...
- Based on [config](/sdk/configure), can show Smart Wallet option on `keys.coinbase.com` popup, `localhost` or not.
