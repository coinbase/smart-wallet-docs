...
If the Smart Wallet popup fails to open, it may be due to the dapp's `Cross-Origin-Opener-Policy`. Be sure to use a directive that allows the Smart Wallet popup to open. 

- ✅ Allows Smart Wallet Popup to open
  - `unsafe-none` (default)
  - `same-origin-allow-popups` (recommended)
- ❌ Blocks the Smart Wallet popup
  - `same-origin`
