...
Because we'd like to save the returned `permissionsContext` in state, we recommend placing all code samples within your page:
...
import { clickAddress } from "abi/Click";

export function Page() {
  const account = useAccount();
  const { grantPermissionsAsync } = useGrantPermissions();
  const [permissionsContext, setPermissionsContext] = useState<string>();
