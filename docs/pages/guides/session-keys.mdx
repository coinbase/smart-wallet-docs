...
```bash [pnpm]
pnpm install viem@npm:@lukasrosario/viem@2.17.4-sessionkeys.5 wagmi@npm:@lukasrosario/wagmi@2.11.1-sessionkeys.5 @coinbase/wallet-sdk
```

```bash [bun]
bun install viem@npm:@lukasrosario/viem@2.17.4-sessionkeys.5 wagmi@npm:@lukasrosario/wagmi@2.11.1-sessionkeys.5 @coinbase/wallet-sdk
```

```bash [yarn]
yarn add viem@npm:@lukasrosario/viem@2.17.4-sessionkeys.5 wagmi@npm:@lukasrosario/wagmi@2.11.1-sessionkeys.5 @coinbase/wallet-sdk
```
...
Additionally, if you don't already have a key management system you plan on using for session keys, we recommend the following version of wevm's `webauthn-p256`. You can use this package to create P-256 key pairs with non-exportable private keys you can sign with and store in the browser (self-custodial). The rest of this guide will use this package, but you can use any other key management system with EOA or P-256 keys you might already have set up.
...
```bash [bun]
bun install webauthn-p256@npm:@lukasrosario/webauthn-p256@0.0.10-sessionkeys.6
```

```bash [yarn]
yarn add webauthn-p256@npm:@lukasrosario/webauthn-p256@0.0.10-sessionkeys.6
```
...
```json [package.json]
"dependencies": {
  "viem": "npm:@lukasrosario/viem@2.17.4-sessionkeys.5",
  "wagmi": "npm:@lukasrosario/wagmi@2.11.1-sessionkeys.5",
...
To enable Session Keys on your web app, we recommend using our experimental `useGrantPermissions` hook in `wagmi`.

The following example also shows how you can `webauthn-p256` to create a P-256 key pair that you can store in the browser (using local state).
            ...
            },
            {
              ...
              data: {
                ...
                selector: toFunctionSelector(
                  // the only function selector that can be called with this permission
...
The `click` function will call the `sendCallsAsync` function from the experimental `useSendCalls` wagmi hook. Since we are calling `sendCallsAsync` with `signatureOverride`, the function will make new RPC requests to the wallet server instead of opening the popup. Specifically, it will:
