...
- post contract addresses here w/ links to basescan? Or a message pointing to the readme such as: For the contract deployment addresses see the contracts [README](https://github.com/coinbase/spend-permissions/blob/main/README.md).
...
```bash
bun add viem wagmi @tanstack/react-query dotenv
```
...
We insecurely use an environment variable in this demo only for simplicity.
</Callout>

Create a local `.env` file, and add two environment variables: `SUBSCRIPTION_PRIVATE_KEY` and `NEXT_PUBLIC_SUBSCRIPTION_SPENDER`.
...
[smart wallet repository](https://github.com/coinbase/smart-wallet) for more details. We'll generate the deterministic address of this Smart Wallet
...
You can run this script to log the counterfactual address of your new smart contract wallet.

<Callout type="info">
You may need to install ts-node and/or typescript if you haven't already: `npm install -g ts-node typescript @types/node`  
You may need to set the value of `"module"` in your `tsconfig.json` to `"commonjs"` to run this script.
</Callout>

Assign this address to the `NEXT_PUBLIC_SUBSCRIPTION_SPENDER` variable in your .env.
