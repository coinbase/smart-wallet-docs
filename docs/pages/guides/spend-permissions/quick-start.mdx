...
In this example, our app spender wallet will be a brand new Coinbase Smart Wallet. This means the wallet itself is a smart
contract, and this smart contract has at least one owner that can produce signatures on behalf of the wallet.
In our case, this owner will be an EOA. Our app will need access to this owner EOA private key, to generate 
signatures, as well as the address of the actual smart contract wallet, which is the spender address actually
being authorized by the spend permission.

The address of a smart contract wallet is deterministic, and depends on the bytecode of the implementation
contract combined with a salt, which in our case will be an array consisting of the initial owner(s) of the smart contract wallet. 
See the [smart wallet repository](https://github.com/coinbase/smart-wallet) for more details about how this works. 
...
The `SPENDER_OWNER_PRIVATE_KEY` will be our spender private key and `NEXT_PUBLIC_SPENDER_WALLET_ADDRESS` will be the address of our spender smart contract wallet.
...
Paste your private key as the value for `SPENDER_OWNER_PRIVATE_KEY`. This should be hex-prefixed, i.e. `SPENDER_OWNER_PRIVATE_KEY=0xAbC123...dEf456`

We'll have to generate the deterministic address of the Smart Wallet owned by this private key so we can store it as a public environment variable.

Paste the following code into a file at the project root called `logSpenderSmartWalletAddress.ts`
...
export async function logSpenderSmartWalletAddress() {
...
async function main() {
  await logSpenderSmartWalletAddress();
}
...
`NEXT_PUBLIC_SPENDER_WALLET_ADDRESS` variable in your .env:

```bash
npm install -g ts-node typescript @types/node dotenv && ts-node logSpenderSmartWalletAddress.ts
```
...
Next, make sure you have a Coinbase Developer Platform Client API key, which you can get [here](https://portal.cdp.coinbase.com/). 
...
In `providers.tsx`, update the value of `keysUrl` to be `"https://keys-beta.coinbase.com/connect"`.
...
export default function Subscribe() {
  ...
  async function handleSubmit() {
    ...
    const spendPermission = {
      ...
      spender: process.env.NEXT_PUBLIC_SPENDER_WALLET_ADDRESS! as Address, // Spender smart contract wallet address
...
...
  ...
  async function handleSubmit() {
    ...
    const spendPermission = {  // [!code focus]
      ...
      spender: process.env.NEXT_PUBLIC_SPENDER_WALLET_ADDRESS! as Address, // Spender smart contract wallet address // [!code focus]
