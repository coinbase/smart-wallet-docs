...
Our spender will need to sign transactions from our app, so we'll create a local wallet for our spender. We'll need a private key and the corresponding ethereum
address in our environment variables. You can use a development private key you already have, or generate a random new one. 
...
Your config in `providers.tsx` should now look like this:
