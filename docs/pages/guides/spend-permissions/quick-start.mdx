...
TODO =============================
- how do we configure this to talk to the chameleon keys deployment for wallet?
- what's the best way for our readers to find and establish their spender address?
    - should we make them create a smart contract wallet first? (if so, how do they get ahold of the private key?)
    - should we have them take our approach and examine error logs to extract the correct spender? (super sloppy)
    - should we provide them with a working example owner private key/ scw address?
- to see this work, they'll need some ETH in their "User" smart contract wallet. we should point them to a faucet?
- post deployment addresses here w/ links to basescan? Or a message pointing to the readme such as: For the contract deployment addresses see the contracts [README](https://github.com/coinbase/spend-permissions/blob/main/README.md).
- consider pointing them to a github permalink for the `Subscribe` component unless we can get these docs working with copy-paste buttons and collapsible codeblocks because it's way too long
- Let's put a link to the complete example project at the top of this guide.  Should we move that project to something hosted in the Coinbase github org?
===================================
...
<Callout type="info">
  You will need a Coinbase Developer Platform API key for this example, which you can get [here](https://portal.cdp.coinbase.com/).  
</Callout>
...
We use an environment variable in this demo only for simplicity.
</Callout>

For this toy example, go ahead and create a local `.env` file, and add two environment variables: `SUBSCRIPTION_PRIVATE_KEY` and `NEXT_PUBLIC_SUBSCRIPTION_SPENDER`.
These will represent our private key and, for convenience, the address of our spender smart contract wallet.

You can generate a random private key [here](https://privatekeys.pw/keys/ethereum/random), but our address won't actually be the corresponding
address listed on this site because our actual spender address will be a smart contract wallet to which this private key is acting as an owner.  
We'll get this smart contract address later.

Next, make sure you have a Coinbase Developer Platform API key, which you can get [here](https://portal.cdp.coinbase.com/). 
Put this in your `.env` as `NEXT_PUBLIC_CDP_API_KEY`.
...
Our client is what our app will use to communicate with the blockchain. In this example, our client will
also be a Coinbase Smart Wallet, and we'll use a [paymaster](/guides/paymasters.mdx) to sponsor our transactions so we don't have to worry
...
```
### Set up our references to the `SpendPermissionManager`
...
<Callout type="info">
[Here's an example](https://docs.basescan.org/api-endpoints/contracts) of finding the ABI of any verified contract on Basescan.
</Callout>
...
Also be sure to add this new component to the top level component on `Page.tsx`. You can delete everything between
the two `<main></main>` tags and simply add `<Subscribe/>`.
...
Run your app locally (`bun run dev`) and visit `localhost:3000`.

If you don't have a Coinbase Smart Wallet yet, the "Connect Wallet" button will bring you to a popup
where you can create one, owned by a passkey stored on your device or in your browser.
