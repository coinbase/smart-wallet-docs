...
about having gas money. Note that EOAs or smart contracts can also be spenders.

For this toy example, go ahead and create two environment variables in a local `.env` called `SUBSCRIPTION_PRIVATE_KEY` and `NEXT_PUBLIC_SUBSCRIPTION_SPENDER`.
...
address listed on this site because our private key will be an owner of a smart contract wallet.  We'll get our smart contract address later.
...
Next, create a sibling directory to `app` called `lib` and add the following `spender.ts` file to create your spender client.

<Callout type="info">
  Your `BASE_SEPOLIA_PAYMASTER_URL` will be  
...
This example shows our spender app approving the permission onchain by submitting the 
...
<Callout type="info">
TODO: post testnet and/or mainnet addresses of manager contract here?  
[Here's an example](https://docs.basescan.org/api-endpoints/contracts) of finding the ABI of a verified contract on Basescan.
</Callout>
...
That's it! We've successfully created, approved and used a spend permission to spend our user's funds.

TODO: link to more comprehensive documentation/complete example app or repo