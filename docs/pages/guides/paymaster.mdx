...
If your app supports other wallets that do not yet support the experimental `writeContracts` action, you will need to fall back to Viem's `writeContract` action. Note that this fallback action does not support supplying a paymaster service URL, so the user will pay the transaction fee in this case.
